...
class ImageScrollView: UIScrollView {
    ...
    // If TRUE, the ImageScrollView will have an 'aspect fill' behavior when in landscape orientation. Otherwise, it will be aspect fit, so that the image isn't zoomed in to fit the width of the display.
    ...
    open func display(image: UIImage) {
        ...
        zoomView?.layer.borderWidth = 1.5
        zoomView?.layer.cornerRadius = 3
        zoomView?.layer.shadowColor = UIColor.black.cgColor
        zoomView?.layer.shadowOffset = CGSize(width: 1, height: 1)
        zoomView?.layer.shadowOpacity = 10
        zoomView?.layer.shadowRadius = 5
